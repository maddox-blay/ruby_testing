...
describe NumberGame do
  ...
  describe '#game_over?' do
    ...
    context 'when user guess is not correct' do
      # remove the 'x' before running this test
      subject(:game_not_over) {described_class.new(1, 3)}
  ...
  describe '#player_turn' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        letter = 'j'
        digit = '7'
        allow(game_loop).to receive(:player_input).and_return(letter, letter, digit)
      end
  ...
  describe '#final_message' do
    ...
    context 'when count is 2-3' do
      # remove the 'x' before running this test
      subject(:game_message) {described_class.new(5, 5, 3)}
      it 'outputs correct phrase' do
        congrats_phrase = "Congratulations! You picked the random number in 3 guesses!\n"
        expect { game_message.final_message }.to output(congrats_phrase).to_stdout
      end
    end
    ...
    context 'when count is 4 and over' do
      subject(:game_count) {described_class.new(5, 5, 5)}
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        congrats_phrase = "That was hard. It took you 5 guesses!\n"
        expect {game_count.final_message}.to output(congrats_phrase).to_stdout
      end
    end
  end
end
